"""Direct Mode harness for contracts/credencelend.py (see support.py for what
...
from tests.direct.support import CONTRACT, MODULE, NOW, register_policy
...
def _tolerate_windows_unlink():
    ...
    if getattr(original, "_credencelend_shim", False):
        return
    ...
    inject_tolerant._credencelend_shim = True
...
@pytest.fixture
def lend(direct_vm, direct_deploy):
...
@pytest.fixture
def genlayer_vm(lend):
    import genlayer
    return genlayer.gl.vm


@pytest.fixture
def policy_id(lend, direct_vm):
    return register_policy(lend, direct_vm)
