#!/usr/bin/env python3
"""Deploy contracts/credencelend.py to GenLayer StudioNet and record the
...
- refuses to deploy unless contracts/credencelend.py is committed and
        ...
        try:
            request = urllib.request.Request(
                RPC, data=body, headers={"Content-Type": "application/json",
                                         "User-Agent": "credencelend-deploy"})
...
def main() -> None:
    ...
    if git("status", "--porcelain", "--", "contracts/credencelend.py"):
        raise SystemExit("refusing: contracts/credencelend.py has uncommitted changes")
    commit = git("log", "-1", "--format=%H", "--", "contracts/credencelend.py")
    head = git("rev-parse", "HEAD")
    blob = git("rev-parse", "HEAD:contracts/credencelend.py")
